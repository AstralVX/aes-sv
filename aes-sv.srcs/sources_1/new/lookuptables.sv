//
// Rijndael S-box - a substitution box (lookup table) used in the Rijndael cipher on which AES is based
...
module rcon(
    ...
    assign byte_o = rcon[byte_i];
endmodule
    
//
// Galois field - 2x multiplication lookup table used for mix columns operation
...
module mul2(
    ...
    assign byte_o = mul2[byte_i];
endmodule

//
// Galois field - 3x multiplication lookup table used for mix columns operation
// https://en.wikipedia.org/wiki/Rijndael_MixColumns
//
// byte_i - byte index into lookup table
// byte_o - result of index into lookup table
